...
public class Easing
{
    ...
    public class Sinusoidal
    {
        public static float In(float k)
        {
            return 1f - Mathf.Cos(k * 3.14f / 2f);
        }

        public static float Out(float k)
        {
            return Mathf.Sin(k * 3.14f / 2f);
        }

        public static float InOut(float k)
        {
            return 0.5f * (1f - Mathf.Cos(3.14f * k));
        }
    };
    ...
    public class Elastic
    {
        public static float In(float k)
        {
            if (k == 0) return 0;
            if (k == 1) return 1;
            return -Mathf.Pow(2f, 10f * (k -= 1f)) * Mathf.Sin((k - 0.1f) * (2f * 3.14f) / 0.4f);
        }
        ...
        public static float Out(float k)
        {
            if (k == 0) return 0;
            if (k == 1) return 1;
            return Mathf.Pow(2f, -10f * k) * Mathf.Sin((k - 0.1f) * (2f * 3.14f) / 0.4f) + 1f;
        }

        public static float InOut(float k)
        {
            if ((k *= 2f) < 1f) return -0.5f * Mathf.Pow(2f, 10f * (k -= 1f)) * Mathf.Sin((k - 0.1f) * (2f * 3.14f) / 0.4f);
            return Mathf.Pow(2f, -10f * (k -= 1f)) * Mathf.Sin((k - 0.1f) * (2f * 3.14f) / 0.4f) * 0.5f + 1f;
        }
    };
